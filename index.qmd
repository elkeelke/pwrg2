...
How can satellite-derived remote sensing data be applied to assess changes in surface albedo resulting from PV installations by comparing its effects before and after the installation?
...
Many satellites provide relevant data (see @sec-satellite-data for a full overview). In this section, the most appropriate are identified. Some satellites, such as MODIS and VIIRS, provide direct albedo monitoring (e.g., MODIS MCD43A3, VIIRS VNP43). However, since the resolution from these satellites is generally coarse with 500 to 750 meters, the resulting data can only be applied for very large PV farms.
...
There is generally a lot of inherent noise found in satellite-derived data due to varying weather conditions and cloud cover.

Nevertheless, it is expected that it will be possible to derive statistically significant result estimates with appropriate data processing and statistical methods.

### Seasonality

Is seasonality (e.g., temporal snow cover and vegetation dynamics) a relevant factor? 
